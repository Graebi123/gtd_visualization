...
predict_and_plot_year <- function(pred_year, features_base, w1, w3, n1, n3) {
  ...
  importance_df <- as.data.frame(importance(rf_model))  # Get IncNodePurity for each variable
  importance_df$Feature <- rownames(importance_df)      # Store variable names as column
  importance_df <- importance_df %>%
    arrange(desc(IncNodePurity)) %>%                    # Sort descending
    select(Feature, IncNodePurity)                      # Clean up columns for reporting
  ...
  # Return both the map and model performance metrics + importance for the year
  return(list(
    year = pred_year,
    map = map,
    R2 = r_squared,
    RMSE = rmse_val,
    MAE = mae_val,
    importance = importance_df  # ← NEW: Return feature importance
  ))
}
...
# Combine and analyze variable importance across years
all_importances <- lapply(final_maps, function(x) {
  x$importance %>% mutate(Year = x$year)
})

importance_combined <- bind_rows(all_importances)

# Average importance per feature across all years
avg_importance <- importance_combined %>%
  group_by(Feature) %>%
  summarise(Mean_IncNodePurity = mean(IncNodePurity)) %>%
  arrange(desc(Mean_IncNodePurity))

# Print average importance
cat("\n=== AVERAGE FEATURE IMPORTANCE (2016–2020) ===\n")
...
#### CREATE PLOT FOR INCIDENTS 2010-2020 ####
...
# Factor to preserve year order and format labels as "'10", "'12", ...
...
plot_incidents <- ggplot(gtd_filtered, aes(x = year, y = n)) +
  geom_col(aes(text = paste0("Incidents: ", n)), fill = "lightgrey") +
  theme_minimal() +
  theme(
...
# Save as HTML
saveWidget(plotly_incidents, file = "www/incidents_2010-2020.html", selfcontained = TRUE)
...
# Combine feature importance for all years
all_importances <- lapply(final_maps, function(x) {
  x$importance %>% mutate(Year = x$year)
})
...
# Print average importance to console
cat("\n=== AVERAGE FEATURE IMPORTANCE (2016–2020) ===\n")
print(avg_importance)

# Plot with Plotly
plotly_importance <- plot_ly(
  data = avg_importance,
  x = ~Mean_IncNodePurity,
  y = ~reorder(Feature, Mean_IncNodePurity),
  type = "bar",
  orientation = "h",
  hoverinfo = "x",
...
# Save plot
saveWidget(plotly_importance, file = "www/feature_importance.html", selfcontained = TRUE)
